# Architecture Decision Records

Design-phase ADRs for INV Meetings. Each resolves a brief item with options, tradeoffs, and a single
recommendation. One-line mirrors live in [`../DECISIONS.md`](../DECISIONS.md); the spine is
[`../DESIGN.md`](../DESIGN.md).

| ADR | Title | Brief item |
|-----|-------|-----------|
| [ADR-001](ADR-001-call-detection-trigger-ux.md) | Call detection & trigger UX | A |
| [ADR-002](ADR-002-capture-pipeline.md) | Capture pipeline (dual-track, no driver) | B |
| [ADR-003](ADR-003-transcription-engine.md) | Transcription engine, biasing, diarization | D |
| [ADR-004](ADR-004-context-assembler.md) | Pre-transcription context assembler | C |
| [ADR-005](ADR-005-context-package-contract.md) | Context package contract & skill integration | F |
| [ADR-006](ADR-006-storage-drive-sync.md) | Storage, backend & Google Drive sync | E |
| [ADR-007](ADR-007-dashboard.md) | Dashboard | E |
| [ADR-008](ADR-008-claude-auto-trigger.md) | Claude auto-trigger & skill chaining | F |
| [ADR-009](ADR-009-app-architecture-ipc.md) | App architecture, language split & IPC | Phase 3 |
| [ADR-010](ADR-010-privacy-consent.md) | Privacy & consent posture | Phase 1/2 |
| [ADR-011](ADR-011-recording-modes.md) | Recording modes: manual capture & in-person meetings | A (extends) |

Status: all **Proposed** — awaiting Yuval's review.
